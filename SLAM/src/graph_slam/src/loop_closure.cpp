...
pair<double, int> scanContextDistance( SCMatrix& A,SCMatrix& B)
{
    ...
    for (int j = 0; j < NS; ++j)
    {
        ...
        if (na > 1e-9) 
        { 
            A_hat.col(j) = (A.col(j) / na).eval();
             a_valid[j] = true; 
        }
        if (nb > 1e-9) 
        { 
            B_hat.col(j) = (B.col(j) / nb).eval(); 
            b_valid[j] = true; 
        }
    }
    ...
    for (int phi = 0; phi < NS; ++phi)
    {
        ...
        for (int j = 0; j < NS; ++j)
        {
            ...
            if (!a_valid[j] || !b_valid[j_shifted]) 
            {
                continue;
            }
...
public:
    LoopClosure() : Node("loop_closure"), lc_in_flight_(0)
    {
        ...
        max_icp_translation_ = declare_parameter("max_icp_translation",5.0);
        ...
        timer_ = create_wall_timer(chrono::duration<double>(period_sec_),[this](){ tick(); },cb_group_reentrant_);

        RCLCPP_INFO(get_logger(),
            "[LoopClosure] max_range=%.1f min_sep=%d candidates=%d "
            "sc_thr=%.2f icp_thr=%.2f max_lc=%d max_trans=%.1f max_world=%.1f",
            max_range_, min_node_separation_, num_candidates_,
            sc_threshold_, icp_threshold_, max_lc_per_tick_, max_icp_translation_, max_world_dist_);
    }
...
private:
    ...
    Eigen::Matrix<double, NR, 1> ringKey(SCMatrix& sc)
    {
    ...
    void updateCaches(vector<PoseNode>& nodes)
    {
        ...
        for (int i = old_size; i < nodes.size(); ++i)
        {
    ...
    vector<int> ringKeyTopK(int q, int num_refs)
    {
        ...
        int K = min(num_candidates_, (int)hits.size());
                             ...
                             vector<pair<int, SCMatrix>> cand_sc_pairs, SCMatrix q_sc)
    {
        ...
        for (int i=0; i < cand_sc_pairs.size(); ++i)
        {
            int ref_idx = cand_sc_pairs[i].first;
            SCMatrix ref_sc = cand_sc_pairs[i].second;
            pair<double, int> dist_shift = scanContextDistance(q_sc, ref_sc);
            
            if (dist_shift.first < best_sc)
            {
                best_sc = dist_shift.first;
                best_ref = ref_idx;
        ...
        double init_dtheta = wrapAngle(nodes[q].theta - nodes[best_ref].theta);
    ...
    {
        bool expected = false;
        if (!tick_in_flight_.compare_exchange_strong(expected, true)) {
            
            return;
        }
        if (lc_in_flight_.load() > 0) {
            tick_in_flight_.store(false);
            return;
        }
        ...
        clt_graph_->async_send_request(req,
            ...
            int N = srv_response->ids.size();
            if (N < min_node_separation_ + 2) {
                tick_in_flight_ = false;
                return;
            }
            if (srv_response->xs.size() < N ||
                srv_response->ys.size() < N ||
                srv_response->thetas.size() < N ||
                srv_response->scans.size() < N) {
                tick_in_flight_ = false;
                return;
            }
            ...
            int lc_dispatched = 0;
            int q_start = min_node_separation_ + 1;
                ...
                SCMatrix q_sc;
                {
                    ...
                    else
                    {
                        ...
                        for (int c : candidates)
                        {
                            ...
                            if (rejected_lc_pairs_.count(lc_pair)) 
                            {
                                continue;
                            }
